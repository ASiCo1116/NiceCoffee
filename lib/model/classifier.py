...
__all__ = ['AdaBoost', 'RF', 'SVM', 'KNN', 'PLSDA', 'DeepClassifier', 'ResNet']
...
class SVM(ConcatenatedML):
    def __init__(self,
        ...
        logging.info('Model: ensemble SVM initial done.')
    ...
    def save(self, path):
        ...
        logging.info('Model: ensemble SVM saving done.')
...
class RF(ConcatenatedML):
    def __init__(self,
        ...
        logging.info('Model: ensemble RF initial done.')
    ...
    def save(self, path):
        ...
        logging.info('Model: ensemble RF saving done.')
    ...
    def _train(self, index, data, label, **kwargs):
        ...
        if isinstance(tree_num, float):
             logging.warning('Argument: tree_num must be int, but got float, force to transfrom the value to int.')
             tree_num = int(tree_num)

        if tree_num == 10:
            logging.warning('Argument: tree_num can be tuned in the training process, please input it for optimal result.')
...
class AdaBoost(ConcatenatedML):
    def __init__(self,
        ...
        # submodel_type can check sklearn API of AdaBoostClassifier
        ...
        self.models = []
        for i in range(self.output_size):
            self.models.append(AdaBoostClassifier(base_estimator = submodel_type))
        ...
        logging.info('Model: ensemble Adaboost initial done.')

    def save(self, path):
        state = {}
        state['model_type'] = 'AdaBoost'
        state['input_size'] = self.input_size
        state['output_size'] = self.output_size
        state['output_descriptions'] = self.output_descriptions
        state['submodel_type'] = self.submodel_type
        state['model_name'] = self.model_name
        state['batch'] = self.batch
        state['preprocess_func'] = self.preprocess_func
        state['models'] = self.models

        save_object(path, state, mode = 'pickle', extension = 'adab')
        logging.info('Model: ensemble AdaBoost saving done.')

        return None

    def load(self, fname):
        if not fname.endswith('.adab'):
            raise RuntimeError('lib.model.AdaBoot only can load file endswith .adab which is saved by this object.')

        state = load_pickle_obj(fname, extension_check = False)

        if state['model_type'] != 'AdaBoost':
            raise RuntimeError('The object is not a state file which is trained by this object.')

        self.input_size = state['input_size']
        self.output_size = state['output_size']
        self.output_descriptions = state['output_descriptions']
        self.submodel_type = state['submodel_type']
        self.model_name = state['model_name']
        self.batch = state['batch']
        self.preprocess_func = state['preprocess_func']
        self.models = state['models']
    ...
    def _train(self, index, data, label, **kwargs):
        max_submodel_num = kwargs.get('max_submodel_num', 50)

        if np.sum(label) == 0:
            self.models[index] = AllZeroModel()
        else:
            self.models[index] = AdaBoostClassifier(base_estimator = self.submodel_type, n_estimators = max_submodel_num)
            self.models[index].fit(data, label)
    ...
    def __repr__(self):
        ...
        if self.submodel_type is None:
            properties_str +=  (' ' * space_length + 'submodel_type: DecisionTreeClassifier(max_depth=1),\n') 
        else:
            properties_str +=  (' ' * space_length + 'submodel_type: ' + self.submodel_type + ',\n')
...
class KNN(ConcatenatedML):
    def __init__(self,
        ...
        if not isinstance(n_neighbors, int):
            raise TypeError('Argument: n_neighbors must be int, please check sklearn API for KNeighborsClassifier.')

        if not isinstance(weights, str):
            raise TypeError('Argument: weights must be str, please check sklearn API for KNeighborsClassifier.')

        weights_available_list = ['uniform', 'distance']
        if weights not in weights_available_list:
            raise ValueError("Argument: weights must be 'uniform' or 'distance'.")
        ...
        self.models = []
        for i in range(self.output_size):
            self.models.append(KNeighborsClassifier(n_neighbors = n_neighbors, weights = weights))
        ...
        self.optimized_properties = {
                ...
                'upper_bound': 20,
                'lower_bound': 1,
                'binary_digit_overflow': 2,
                'type_function': int},
                }

        logging.info('Model: ensemble KNN initial done.')
    ...
    def save(self, path):
        ...
        save_object(path, state, mode = 'pickle', extension = 'knn')
        logging.info('Model: ensemble KNN saving done.')

        return None

    def load(self, fname):
        if not fname.endswith('.knn'):
            raise RuntimeError('lib.model.KNN only can load file endswith .knn which is saved by this object.')

        state = load_pickle_obj(fname, extension_check = False)

        if state['model_type'] != 'KNN':
            raise RuntimeError('The object is not a state file which is trained by this object.')
    ...
    def _train(self, index, data, label, **kwargs):
        ...
        if isinstance(n_neighbors, float):
             logging.warning('Argument: n_neighbors must be int, but got float, force to transfrom the value to int.')
             n_neighbors = int(n_neighbors)

        if n_neighbors == 5:
            logging.warning('Argument: n_neighbors can be tuned in the training process, please input it for optimal result.')

        if np.sum(label) == 0:
            self.models[index] = AllZeroModel()
        else:
            self.models[index] = KNeighborsClassifier(n_neighbors = n_neighbors, weights = self.weights)
            self.models[index].fit(data, label)
    ...
    def __repr__(self):
        ...
        properties_str +=  (' ' * space_length + 'input_size: ' + str(self.input_size) + ',\n')
        properties_str +=  (' ' * space_length + 'output_size: ' + str(self.output_size) + ',\n')
        properties_str +=  (' ' * space_length + 'n_neighbors: ' + self.n_neighbors + ',\n')
        properties_str +=  (' ' * space_length + 'weight: ' + self.weight + ',\n')
        properties_str +=  (' ' * space_length + 'batch: ' + str(self.batch) + ')')

        return self.__class__.__name__ + properties_str


class PLSDA(ConcatenatedML):
    def __init__(self,
            input_size = (),
            output_size = 0,
            n_components = 2,
        ...
        if not isinstance(n_components, int):
            raise TypeError('Argument: n_components must be int, please check sklearn API for PLSRegression.')

        if not isinstance(threshold, float):
            raise TypeError('Argument: threshold must be float.')
        ...
        self.models = []
        for i in range(self.output_size):
            self.models.append(PLSRegression(n_components = n_components))
        ...
        logging.info('Model: ensemble PLSDA initial done.')
    ...
    def predict(self, data):
        ...
        for i in range(len(self.models)):
            output[:, i] = np.squeeze(self.models[i].predict(data), axis = 1)
    ...
    def single_predict(self, data, index = 0):
        ...
        output = np.zeros(output_size)
        output[:, 0] = np.squeeze(self.models[index].predict(data), axis = 1)
    ...
    def save(self, path):
        ...
        save_object(path, state, mode = 'pickle', extension = 'plsda')
        logging.info('Model: ensemble PLSDA saving done.')

        return None

    def load(self, fname):
        if not fname.endswith('.plsda'):
            raise RuntimeError('lib.model.PLSDA only can load file endswith .knn which is saved by this object.')

        state = load_pickle_obj(fname, extension_check = False)

        if state['model_type'] != 'PLSDA':
            raise RuntimeError('The object is not a state file which is trained by this object.')
    ...
    def _train(self, index, data, label, **kwargs):
        ...
        if isinstance(n_components, float):
             logging.warning('Argument: n_components must be int, but got float, force to transfrom the value to int.')
             n_components = int(n_components)

        if n_components == 2:
            logging.warning('Argument: n_components can be tuned in the training process, please input it for optimal result.')

        if np.sum(label) == 0:
            self.models[index] = AllZeroModel()
        else:
            self.models[index] = PLSRegression(n_components = n_components)
    ...
    def __repr__(self):
        ...
        properties_str +=  (' ' * space_length + 'input_size: ' + str(self.input_size) + ',\n')
        properties_str +=  (' ' * space_length + 'output_size: ' + str(self.output_size) + ',\n')
        properties_str +=  (' ' * space_length + 'n_components: ' + self.n_components + ',\n')
        properties_str +=  (' ' * space_length + 'weight: ' + self.weight + ',\n')
        properties_str +=  (' ' * space_length + 'batch: ' + str(self.batch) + ')')
...
class DeepClassifier(DeepLearningModule):
    ...
    def save(self, path):
        ...
        logging.info('Model: DeepClassifier saving done.')
